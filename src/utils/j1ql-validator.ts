  ...
  async validateQuery(query: string): Promise<ValidationResult> {
    try {
      ...
      return {
        ...
        suggestion:
          'Verify that entities matching your criteria exist. Try removing filters or using broader entity classes.',
      };
    } catch (error) {
  ...
  private getErrorPatterns() {
    return [
      ...
      },
      {
        ...
        suggestion:
          'Direction arrows must follow the relationship verb: "THAT HAS >>" not "THAT >>"',
      },
      {
        pattern: /Error parsing query\. Unexpected token ">" at line \d+ column \d+.*=>/i,
      ...
      {
        pattern:
          /Error parsing query\. Unexpected token "WITH" at line \d+ column \d+\. Did you mean "with"\?/i,
        suggestion: 'Place alias after WITH: "WITH property = value AS alias"',
      },
      // Existing patterns that still work with queryV2
      {
        pattern:
          /Invalid return selector provided: "(\w+)", valid return selectors are: "([^"]+)"/i,
        suggestion: (match: RegExpMatchArray) =>
          `Undefined alias "${match[1]}". Use "${match[2]}" or define alias: FIND ${match[2]} AS ${match[1]}`,
      },
      {
        pattern:
          /Invalid predicate filter selector provided: "(\w+)", valid predicate selectors are: "([^"]+)"/i,
        suggestion: (match: RegExpMatchArray) =>
          `Undefined alias "${match[1]}" in WHERE clause. Aliases in WHERE must be defined earlier. Valid selectors: ${match[2]}`,
      },
      {
      ...
      },
      {
        ...
        suggestion:
          'Query returned too much data. Add LIMIT clause to reduce result size (e.g., LIMIT 100)',
      },
      {
        pattern: /Invalid entity type or class/i,
        suggestion:
          'Check entity class capitalization (e.g., User not user) and type format (e.g., aws_iam_user).',
      },
      {
        pattern: /Unknown property/i,
        suggestion:
          'Property does not exist. Run discovery: FIND <EntityClass> AS e RETURN e.* LIMIT 10',
      },
      {
  ...
  public analyzeSyntaxError(query: string, _errorMessage: string): string {
    const syntaxChecks = [
      ...
      },
      {
        ...
        message:
          query.match(/\bLIMIT\s+\d+/i) || query.match(/\bCOUNT\s*\(/i)
            ? null
      ...
      {
        pattern:
          /\bAS\s+(count|sum|avg|min|max|find|that|with|where|return|order|by|limit|skip|has|relates|to|from|and|or|not|true|false|null|undefined|as)\b/i,
        message: (match: RegExpMatchArray) => {
